...
func TestHTTPLogs(t *testing.T) {
	...
	log := models.NewAirtimeTransferredLog(testdata.Org1.ID, "http://foo.bar", 200, "GET /", "STATUS 200", false, time.Second, 0, time.Now())
	err := models.InsertHTTPLogs(ctx, rt.DB, []*models.HTTPLog{log})
	assert.Nil(t, err)

	assertdb.Query(t, rt.DB, `SELECT count(*) from request_logs_httplog WHERE org_id = $1 AND status_code = 200 AND is_error = FALSE`, testdata.Org1.ID).Returns(1)

	// insert a log with nil response
	log = models.NewAirtimeTransferredLog(testdata.Org1.ID, "http://foo.bar", 0, "GET /", "", true, time.Second, 0, time.Now())
	err = models.InsertHTTPLogs(ctx, rt.DB, []*models.HTTPLog{log})
	assert.Nil(t, err)

	assertdb.Query(t, rt.DB, `SELECT count(*) from request_logs_httplog WHERE org_id = $1 AND status_code = 0 AND is_error = TRUE AND response IS NULL`, testdata.Org1.ID).Returns(1)
