...
func TestBulkTimeout(t *testing.T) {
		...
		Timeouts: []*timeouts.Timeout{
			{
				...
				TimeoutOn:  time.Date(2025, 1, 1, 11, 0, 0, 0, time.UTC),
				ModifiedOn: time.Date(2024, 11, 15, 13, 57, 0, 0, time.UTC),
			},
			{
				SessionID:  234567,
				ContactID:  testdata.Bob.ID,
				TimeoutOn:  time.Date(2025, 1, 1, 12, 0, 0, 0, time.UTC),
	...
	testsuite.AssertContactTasks(t, testdata.Org1, testdata.Cathy, []string{
		`{"type":"timeout_event","task":{"session_id":123456,"time":"2025-01-01T11:00:00Z","modified_on":"2024-11-15T13:57:00Z"},"queued_on":"2024-11-15T13:59:00Z"}`,
	})
	testsuite.AssertContactTasks(t, testdata.Org1, testdata.Bob, []string{
		`{"type":"timeout_event","task":{"session_id":234567,"time":"2025-01-01T12:00:00Z","modified_on":"2024-11-15T13:58:00Z"},"queued_on":"2024-11-15T13:59:00Z"}`,
	})
}
