...
func TestResume(t *testing.T) {
	...
	sessions, err := runner.StartFlow(ctx, rt, oa, flow, []*models.Contact{modelContact}, []flows.Trigger{trigger}, nil, true, models.NilStartID, nil, nil)
	...
	for i, tc := range tcs {
		...
		session, err = runner.ResumeFlow(ctx, rt, oa, session, modelContact, resume, nil, nil, nil)
		assert.NoError(t, err)
